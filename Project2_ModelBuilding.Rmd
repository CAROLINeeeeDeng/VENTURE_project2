...
model_A <- lmer(Total.ST.min ~ offset(log(Pickups)) + log(lag_TSM) + Intervention + weekday + pets + sex + age + course.credit + siblings + apps + devices +procrastination.score + (1 | pseudo_id), data = data_A)
...
model_B <- lmer(Total.ST.min ~ offset(log(Pickups)) + log(lag_TSM) + Intervention + weekday + pets + sex + age + course.credit + siblings + apps + devices +procrastination.score + (1 | pseudo_id), data = data_B)
...
# Summaries of p-values and correlations
summary(model_A)
summary(model_B)
```
